<template>
    <div class="service-list relative">
        <div class="w-full h-full flex flex-center"  v-if="services.length == 0">
            <div class="w-full flex flex-col items-center">
                <img class="add-first-service mb-16 cursor-pointer" src="../../assets/images/add-first-service-btn.png" alt="" @click="addService()">
                <p class="text-24px text-yellow text-center">A category has at least one service!</p>
            </div>
        </div>
        <div class="" v-else>
            <div class="service-header">
                ...
                <div class="flex items-center">
                    <p class="text-16px font-semibold uppercase">SERVICE NAME</p>
                </div>
                <div class="flex flex-center">
                    <p class="text-16px font-semibold uppercase">PRICE</p>
                </div>
                <div class="flex flex-center">
                    <p class="text-16px font-semibold uppercase">DURATION</p>
                </div>
                <div class="flex items-center">
                    <p class="text-16px font-semibold uppercase">ACTION</p>
                </div>
            </div>
            <vue-draggable-next class="dragArea list-group w-full" :list="services">
                <div class="mb-5"  :label="item.id" v-for="item in services" :key="item.id">
                    <div class="item w-full cursor-pointer " :class="serviceSelectedId == item.id ? 'bg-purple-ct text-white' : ''">
...
<script setup>
    ...
    const servicesList = computed(() => store.getservicesChecked)
    const services = computed(() => store.getCurrentService)
    const emit = defineEmits(['openAddService', 'openEditService'])
    ...
    // CREATE DATA
    const servicesListChecked = ref([])

    const serviceSelectedId = ref(-1)
...
.service-list{
    ...
    .service-header{
        ...
        margin-bottom: 30px;
        display: grid;
        grid-template-columns:   48px auto   200px 200px  70px;
        @media (max-width: 1300px) {
        grid-template-columns:   48px auto   150px 150px  70px;
        }
         @media (max-width: 1000px) {
        grid-template-columns:   48px auto   100px 100px  50px;
        p{
            padding: 0 0.5rem;
        }
        }
    ...
    .item{
        ...
        display: grid;
        grid-template-columns: 48px  auto  200px 200px 38px 32px;
        @media (max-width: 1300px) {
        grid-template-columns:   48px auto   150px 150px  38px 32px;
        }
        @media (max-width: 1000px) {
        grid-template-columns:   48px auto   100px 100px  28px 22px;
        p{
            padding: 0 0.5rem;
        }
        }
