<template>
    <div class="categories-list">
        <div class="">
            <vue-draggable-next class="dragArea list-group w-full" :list="categoriesList">
                <div class="mb-5"  :label="item.id" v-for="item in categoriesList" :key="item.id" @click="selected(item.id)">
                    <div class="item w-full cursor-pointer " :class="categorySelectedId == item.id ? 'bg-purple-ct text-white' : ''">
                        <div class="h-full flex items-center justify-end">
                            <svg width="14" height="14" viewBox="0 0 14 14" fill="none" xmlns="http://www.w3.org/2000/svg">
                                ...
                                :fill="categorySelectedId == item.id ? '#fff' : '#030504'"/>
                            </svg>
                        </div>
...
<script setup>
    ...
    const categorySelectedId = computed(() => store.getCurrentCategoryId)

    const emit = defineEmits(['openEditCategory'])


    onMounted(() => {
    if(categoriesList.value.length > 0){
        store.setCurrentCategory(categoriesList.value[0].id)
        }
    })
